...
- 프레임 judge: 로컬 멀티모달 LLM(`qwen3-vl:8b` 기본, 사양별 선택 가능) 기반 VQA
- 역명/OCR: 기본 VLM OCR(`qwen3-vl:8b` 기본) + 선택형 역명 사전 보정
...
지정한 경로의 `mp4`, `avi`, `mov`, `mkv` 영상을 일정 간격으로 샘플링하고, 로컬 Qwen VL 모델로 전차선/가공전차선 주변 지장수목 의심 프레임을 판정합니다.
